...
class SimpleEquipmentSimulator:
    ...
    def populate_equipment(self):
        ...
        # Define some problematic equipment (AFC-007, AFC-008, AFC-009, AFC-010 fully operational)
        problems = {
    ...
    def run(self):
        ...
        try:
            ...
            # Update restaurant status based on equipment
            print("🔄 Updating restaurant status...")
            self.update_restaurant_status()
            print()
            
            # Create inventory and staffing gaps for critical restaurants
            print("🔧 Creating inventory and staffing gaps for critical restaurants...")
            self.create_gaps()
            print()
            ...
            print("   • Inventory gaps created for AFC-001, AFC-002, AFC-003")
            print("   • Staffing gaps created for AFC-001, AFC-002, AFC-003")
            print()
    ...
    def create_gaps(self):
        ...
        critical_restaurants = ['AFC-001', 'AFC-002', 'AFC-003']
        
        # Create inventory gaps
        print("🔧 Creating inventory gaps for critical restaurants...")
        inventory_table = self.dynamodb.Table('rest-monitor-inventory-items-prod')
        
        for restaurant_id in critical_restaurants:
            response = inventory_table.query(
                KeyConditionExpression='restaurant_id = :rid',
                ExpressionAttributeValues={':rid': restaurant_id}
            )
            items = response.get('Items', [])
            critical_count = int(len(items) * 0.3)
            low_count = int(len(items) * 0.4)
            
            for i, item in enumerate(items):
                reorder_point = float(item.get('reorder_point', 100))
                if i < critical_count:
                    new_qty = Decimal(str(round(reorder_point * random.uniform(0, 0.5), 2)))
                elif i < critical_count + low_count:
                    new_qty = Decimal(str(round(reorder_point * random.uniform(0.5, 0.99), 2)))
                else:
                    new_qty = Decimal(str(round(reorder_point * random.uniform(1.0, 2.0), 2)))
                
                inventory_table.update_item(
                    Key={'restaurant_id': item['restaurant_id'], 'item_id': item['item_id']},
                    UpdateExpression='SET current_quantity = :qty',
                    ExpressionAttributeValues={':qty': new_qty}
                )
            print(f"✅ {restaurant_id}: {critical_count} critical, {low_count} low")
        
        # Create staffing gaps
        print("🔧 Creating staffing gaps for critical restaurants...")
        sched_table = self.dynamodb.Table('rest-monitor-staffing-schedule-prod')
        today = date.today().isoformat()
        
        for restaurant_id in critical_restaurants:
            response = sched_table.query(
                KeyConditionExpression='restaurant_id = :rid',
                FilterExpression='shift_date = :date',
                ExpressionAttributeValues={':rid': restaurant_id, ':date': today}
            )
            shifts = response.get('Items', [])
            delete_count = int(len(shifts) * 0.6)
            
            for shift in shifts[:delete_count]:
                sched_table.delete_item(
                    Key={'restaurant_id': shift['restaurant_id'], 'shift_id': shift['shift_id']}
                )
            print(f"✅ {restaurant_id}: Removed {delete_count}/{len(shifts)} shifts (60% gap)")
    
    def update_restaurant_status(self):
        """Update restaurant status based on equipment, inventory AND staffing status"""
        # Get equipment status
        equipment_response = self.equipment_table.scan()
        equipment_by_restaurant = {}
        
        for item in equipment_response['Items']:
            restaurant_id = item['restaurant_id']
            status = item.get('status', 'operational')
            
            if restaurant_id not in equipment_by_restaurant:
                equipment_by_restaurant[restaurant_id] = {'critical': 0, 'warning': 0, 'operational': 0}
            
            if status == 'critical':
                equipment_by_restaurant[restaurant_id]['critical'] += 1
            elif status == 'warning':
                equipment_by_restaurant[restaurant_id]['warning'] += 1
            else:
                equipment_by_restaurant[restaurant_id]['operational'] += 1
        ...
        try:
            ...
            for item in inventory_response['Items']:
                restaurant_id = item['restaurant_id']
                quantity = float(item.get('current_quantity', 0))
                reorder_point = float(item.get('reorder_point', 0))
                
                if restaurant_id not in inventory_by_restaurant:
                    inventory_by_restaurant[restaurant_id] = {'critical': 0, 'low': 0, 'ok': 0}
                
                if quantity == 0:
                    inventory_by_restaurant[restaurant_id]['critical'] += 1
                elif quantity < reorder_point * 0.5:
                    inventory_by_restaurant[restaurant_id]['critical'] += 1
                elif quantity < reorder_point:
                    inventory_by_restaurant[restaurant_id]['low'] += 1
                else:
                    inventory_by_restaurant[restaurant_id]['ok'] += 1
        except Exception as e:
            print(f"⚠️  Could not check inventory status: {e}")
            inventory_by_restaurant = {}
        
        # Get staffing status
        try:
            from datetime import date
            staffing_req_table = self.dynamodb.Table('rest-monitor-staffing-requirements-prod')
            staffing_sched_table = self.dynamodb.Table('rest-monitor-staffing-schedule-prod')
            today = date.today().isoformat()
            
            staffing_by_restaurant = {}
            
            for restaurant_id in self.locations.keys():
                # Get requirements
                req_response = staffing_req_table.get_item(Key={'restaurant_id': restaurant_id, 'date': today})
                if 'Item' not in req_response:
                    continue
                    
                required_roles = req_response['Item'].get('roles', {})
                total_required = sum(int(v) for v in required_roles.values())
                
                # Get scheduled staff
                sched_response = staffing_sched_table.query(
                    KeyConditionExpression='restaurant_id = :rid',
                    FilterExpression='shift_date = :date',
                    ExpressionAttributeValues={':rid': restaurant_id, ':date': today}
                )
                total_scheduled = len(sched_response.get('Items', []))
                
                gap_pct = ((total_required - total_scheduled) / total_required * 100) if total_required > 0 else 0
                
                staffing_by_restaurant[restaurant_id] = {
            ...
            print(f"⚠️  Could not check staffing status: {e}")
            staffing_by_restaurant = {}
        
        # Update restaurant status based on equipment, inventory AND staffing
        for restaurant_id in equipment_by_restaurant.keys():
            equipment = equipment_by_restaurant.get(restaurant_id, {})
            inventory = inventory_by_restaurant.get(restaurant_id, {})
            staffing = staffing_by_restaurant.get(restaurant_id, {})
            
            # Critical if equipment/inventory critical OR staffing gap > 50%
            if equipment.get('critical', 0) > 0 or inventory.get('critical', 0) > 0 or staffing.get('gap_pct', 0) > 50:
                status = 'critical'
                status_color = 'red'
            # Warning if equipment/inventory warning OR staffing gap 20-50%
            elif equipment.get('warning', 0) > 0 or inventory.get('low', 0) > 0 or staffing.get('gap_pct', 0) > 20:
                status = 'warning'
                status_color = 'yellow'
            else:
                status = 'operational'
            ...
            self.restaurants_table.update_item(
                ...
                UpdateExpression='SET #status = :status, status_color = :color',
                ExpressionAttributeNames={'#status': 'status'},
                ExpressionAttributeValues={':status': status, ':color': status_color}
            )
        
        print("✅ Restaurant statuses updated based on equipment, inventory and staffing")
